...
async function main(){
 ...
 const chapters=data.surahs.map((s,i)=>{
  ...
  if(s.number!==i+1||s.ayahs?.length!==C.COUNTS[i])throw Error('Invalid Quran chapter.');
  return s;
 });
 const edition=C.validateCorpus({code:200,data:{...data,surahs:chapters}},'quran-uthmani');
