/* NUR AI key vault. No plaintext key is persisted or included in NUR backups. */
...
function createStore(env={}){
 ...
 const get=()=>record('readonly',s=>s.get(ID));const put=x=>record('readwrite',s=>s.put(x,ID));const remove=()=>record('readwrite',s=>s.delete(ID));
 ...
 function setSession(value){const v=String(value||'').trim();if(!v||v.length>512||/[\s]/.test(v))throw Error('Enter a valid API key.');key=v;return true;}
 async function save(value,password){const v=String(value||'').trim();if(typeof password!=='string'||password.length<10)throw Error('Use a passphrase of at least 10 characters.');if(!crypto?.subtle||!crypto.getRandomValues)throw Error('Encrypted storage is not supported.');if(!v||v.length>512||/\s/.test(v))throw Error('Enter a valid API key.');const salt=crypto.getRandomValues(new Uint8Array(16)),iv=crypto.getRandomValues(new Uint8Array(12));const derived=await derive(password,salt);const cipher=await crypto.subtle.encrypt({name:'AES-GCM',iv},derived,enc.encode(v));await put({version:1,salt:b64(salt),iv:b64(iv),cipher:b64(new Uint8Array(cipher))});key=v;return true;}
 ...
 async function clear(){await remove();key='';return true;}
 function lock(){key='';}
